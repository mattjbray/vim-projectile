...
function! s:open_projection(cmd, variants, ...) abort
  ...
  if a:0
    ...
    let base = matchstr(a:1, '.*/\zs.*')
    call map(formats, 'substitute(substitute(v:val, "\\*\\*", dir, ""), "\\*", base, "")')
  else
    call filter(formats, 'v:val !~# "\\*"')
  endif
  if empty(formats)
...
function! s:projection_complete(lead, cmdline, _) abort
  ...
  for format in s:last_formats
    ...
    let [before, middle, after] = split(format, '\*\*\=', 1)
    let matches = map(split(glob(format), "\n"), 's:slash(v:val)')
    let results += map(matches, 'substitute(v:val[strlen(before) : -strlen(after)-1], ".*\\zs\\M".middle, "/", "")')
  endfor
